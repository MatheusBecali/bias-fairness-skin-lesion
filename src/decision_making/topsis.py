...
class TOPSIS:
    ...
    def plot_ranking(self, alt_names=None, save_path=None, show=True):
        ...
        if alt_names is not None:
            # Trocar x por y e vice-versa, adicionar orient='h'
            sorted_indices = np.argsort(self.clos_coefficient)[::-1]  # [::-1] inverte para decrescente
            sorted_names = np.array(alt_names)[sorted_indices]
            sorted_coeffs = np.array(self.clos_coefficient)[sorted_indices]
            
            # Plot com dados ordenados
            a = sns.barplot(
                y=sorted_names, 
                x=sorted_coeffs, 
                hue=sorted_names, 
            ...
            # Trocar x por y e vice-versa, adicionar orient='h'
            a = sns.barplot(y=temp, x=self.clos_coefficient, hue=temp, 
                            palette="BuGn_d", legend=False, orient='h')

        # Trocar os labels também
        a.set_xlabel("Closeness Coefficient")
